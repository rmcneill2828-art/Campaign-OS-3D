...
const SYSTEM_PROMPT = [
  ...
  "",
  "Use remove_token to delete a token from the encounter outright -- a summoned creature",
  "expiring, an NPC leaving the scene for good, cleaning up a temporary/mistaken spawn. When",
  "narration alone implies a token should stop existing, prefer NOT using this over a dead",
  "token that just stays on the map, visibly dead (usually what narration actually wants,",
  "since it's permanent and undoable, unlike toggle_condition's hiddenFromPlayers). But if the",
  "DM's command directly and explicitly asks to remove/delete a specific named token --",
  "\"remove Skeleton 1\", \"delete that token\" -- just issue the action; an explicit instruction",
  "like that is never a case for second-guessing or narrating around it instead.",
  "",
